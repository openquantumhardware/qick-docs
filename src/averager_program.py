    ...
    def acquire(self, soc, load_pulses=True, ReadoutPerExpt=1, SaveExperiments=[0], progress=True, debug=False):
        ...
        if SaveExperiments == []:
            ...
            for nn, ii in enumerate(SaveExperiments):
                avg_di[0][nn] = np.sum(di_buf[0][ii::ReadoutPerExpt].reshape((expts, reps)), 1) / (reps) / \
                                self.cfg['adc_lengths'][0]
                avg_dq[0][nn] = np.sum(dq_buf[0][ii::ReadoutPerExpt].reshape((expts, reps)), 1) / (reps) / \
                                self.cfg['adc_lengths'][0]
                avg_amp[0][nn] = np.sqrt(avg_di[0][nn] ** 2 + avg_dq[0][nn] ** 2)

                avg_di[1][nn] = np.sum(di_buf[1][ii::ReadoutPerExpt].reshape((expts, reps)), 1) / (reps) / \
                                self.cfg['adc_lengths'][0]
                avg_dq[1][nn] = np.sum(dq_buf[1][ii::ReadoutPerExpt].reshape((expts, reps)), 1) / (reps) / \
                                self.cfg['adc_lengths'][0]
