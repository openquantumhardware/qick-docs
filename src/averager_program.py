...
class AveragerProgram(QickProgram):
    ...
    def acquire_round(self, soc, threshold=None, angle=[0, 0], readouts_per_experiment=1, save_experiments=[0],
                      load_pulses=True, progress=True, debug=False):
        """
        ...
        while count < total_count:  # Keep streaming data until you get all of it
            ...
            if count >= min(last_count + 1000,
                            total_count - 1):  # wait until either you've gotten 1000 measurements or until you've finished (so you don't go crazy trying to download every measurement
        ...
        for nn, ii in enumerate(save_experiments):
            for ch in range(2):
                if threshold is None:
                    avg_di[ch][nn] = np.sum(di_buf[ch][ii::readouts_per_experiment]) / (reps) / self.cfg['adc_lengths'][
                        ch]
                    avg_dq[ch][nn] = np.sum(dq_buf[ch][ii::readouts_per_experiment]) / (reps) / self.cfg['adc_lengths'][
                        ch]
                else:
    ...
    def acquire(self, soc, threshold=None, angle=[0, 0], readouts_per_experiment=1, save_experiments=[0],
                load_pulses=True, progress=True, debug=False):
        """
        This method optionally loads pulses on to the SoC, configures the ADC readouts, loads the machine code representation of the AveragerProgram onto the SoC, starts the program and streams the data into the Python, returning it as a set of numpy arrays.
        config requirements:
        ...
        if "rounds" not in self.cfg or self.cfg["rounds"] == 1:
            return self.acquire_round(soc, threshold=threshold, angle=angle, load_pulses=load_pulses, progress=progress,
                                      debug=debug)

        avg_di = None
        for ii in tqdm(range(self.cfg["rounds"]), disable=not progress):
            avg_di0, avg_dq0, avg_amp0 = self.acquire_round(soc, threshold=threshold, angle=angle,
                                                            load_pulses=load_pulses, progress=False, debug=debug)
    ...
    def get_single_shots(self, di, dq, threshold, angle=[0, 0]):
        ...
        return np.array([np.heaviside(
            (di[ch] * np.cos(angle[ch]) - dq[ch] * np.sin(angle[ch])) / self.cfg['adc_lengths'][ch] - threshold[ch], 0)
                         for ch in range(2)])
...
class RAveragerProgram(QickProgram):
    ...
    def acquire_round(self, soc, threshold=None, angle=[0, 0], readouts_per_experiment=1, save_experiments=[0],
        ...
        for nn, ii in enumerate(save_experiments):
            for ch in range(2):
                if threshold is None:
                    avg_di[ch][nn] = np.sum(di_buf[ch][ii::readouts_per_experiment].reshape((expts, reps)), 1) / (
                        reps) / self.cfg['adc_lengths'][ch]
                    avg_dq[ch][nn] = np.sum(dq_buf[ch][ii::readouts_per_experiment].reshape((expts, reps)), 1) / (
                        reps) / self.cfg['adc_lengths'][ch]
                else:
                    avg_di[ch][nn] = np.sum(self.shots[ch][ii::readouts_per_experiment].reshape((expts, reps)), 1) / (
                        reps)
                    avg_dq = np.zeros(avg_di.shape)
    ...
    def acquire(self, soc, threshold=None, angle=[0, 0], load_pulses=True, readouts_per_experiment=1,
                save_experiments=[0], progress=True, debug=False):
        """
        This method optionally loads pulses on to the SoC, configures the ADC readouts, loads the machine code representation of the AveragerProgram onto the SoC, starts the program and streams the data into the Python, returning it as a set of numpy arrays.
        config requirements:
        "reps" = number of repetitions;
        ...
        if "rounds" not in self.cfg or self.cfg["rounds"] == 1:
            return self.acquire_round(soc, threshold=threshold, angle=angle,
                                      readouts_per_experiment=readouts_per_experiment,
                                      save_experiments=save_experiments, load_pulses=load_pulses, progress=progress,
                                      debug=debug)
